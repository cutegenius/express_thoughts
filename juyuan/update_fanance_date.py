...
save_path = os.path.join(date_dair, 'download_from_juyuan')
...
#     eps = data.basiceps
#     netasset = data.netassetps
#
#     # 把eps和netasset调整为日度数据
#     eps = adjust_months(eps)
#     netasset = adjust_months(netasset)
#
#     eps = append_df(eps, target_feq='D')
#     netasset = append_df(netasset, target_feq='D')
#
#     # 对close和eps、netasset取都存在的股票和日期
#     close, eps, netasset = align(close, eps, netasset)
#
#     # 相除得到pe、pb
#     bp = netasset/close
#     ep = eps/close
#
#     # 存储
#     ep.to_csv(os.path.join(save_path, 'ep_daily.csv'), encoding='gbk')
#     bp.to_csv(os.path.join(save_path, 'bp_daily.csv'), encoding='gbk')
#
#     compute_changepect_open_daily()
...
def update_all_basic():
    ...
    factor_dict = {
                   ...
                   # 'LC_SuspendResumption':            # 停复牌
                   # 说明：聚源的这个表有点复杂，还需要进一步处理，不像Wind的停牌信息那么好用。
                   # 就不用这个了。而且停牌时价格是空的，可以直接使用这个来做 Isopen的判断。
                   #      ['InnerCode',
                   #       'SuspendDate',               # 停牌日期
                   #       'SuspendTime',               # 停牌时间
                   #       'ResumptionDate',            # 复牌日期
                   ...
                   #         'ClosePrice',
                   #         'ChangePCT',
                   #         'TurnoverVolume',  # 成交量(万股)
                   #         'TurnoverValue',   # 成交金额(万元)
                   #         'TurnoverRate',    # 换手率
                   #         'TotalMV',         # 总市值(万元)    decimal(19, 4)
    ...
    basic_list = ['CompanyCode', 'InnerCode', 'EndDate', 'TradingDay']    # 'IfMerged', 'IfAdjusted'
