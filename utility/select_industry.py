...
def my_factor_concat(path_dict, factors_to_concat, new_factor_name, concat_type='equal_weight',
                     icir_window=6):
    """
    因子合成：
    ...
    old_matrix_path = path_dict['old_matrix_path']
    ...
    if concat_type == 'max_ic_ir' and len(factors_to_concat) > 1:
        # IC里面存的是_div_
        max_icir_weight_pd = weight_optimizer(factors_to_concat, ic_path, icir_window)
        new_col = [f.replace('_div_', '/') for f in max_icir_weight_pd.columns]
        max_icir_weight_pd.columns = new_col

    factors_to_concat = [f.replace('_div_', '/') for f in factors_to_concat]

    if concat_type == 'max_ic_ir' and len(factors_to_concat) > 1:
        zipped = zip(factors_to_concat, max_icir_weight_pd.columns)
        for (z1, z2) in zipped:
            if z1 != z2:
                print('名字不对')
                raise Exception
    ...
    for f in range(icir_window+1, len(dirlist)):
        fin = dirlist[f]
        # 依次打开每个月度数据
        ...
        # 使用相应的合成方式合成因子
        if len(factors_to_concat) == 1:     # 仅是重命名
        ...
        elif concat_type == 'max_ic_ir':
            # 主要是计算IC值时使用的一年一年的计算，在2019年还没算IC值。
            if fin.split('.')[0] in max_icir_weight_pd.index:
                wei = max_icir_weight_pd.loc[fin.split('.')[0], :]
                factor_concated = factor_dat.apply(apply_func, args=(concat_type, wei), axis=1)
            else:
                continue
        ...
        if panelfactors:
            ...
            panel_dat = pd.read_csv(os.path.join(cpanel_spath, fin), encoding='gbk',
                                    engine='python', index_col=[0])
            ...
            # 如果不存在，则新建一个新的
            panel_dat = pd.concat([dat[tmp_cols], factor_concated], axis=1)
        # 存储截面数据
